...
class RegisterEmailService
{
    ...
    public function __construct(
        ...
        if (!isset($_SESSION['register_requests'])) {
            $_SESSION['register_requests'] = [];
        }
    }

    public function createRequest(string $username, string $email, string $password): void
    {
        $code = random_int(self::MIN_CODE, self::MAX_CODE);

        $_SESSION['register_requests'][$email] = [
            'username' => $username,
            'email'    => $email,
            'password' => $password,
            'code'     => $code,
        ...
        <div style="font-family: Arial, sans-serif; text-align: center; padding: 20px;">
            <h1>Confirmer la création de votre compte</h1>
        ...
        $this->resendHelper->send(
            $email,
            'Confirmer la création de votre compte CollectoVerse',
            $html
        );
    }
    ...
    {
        if (!isset($_SESSION['register_requests'][$email])) {
            throw new Exception('Le code entré est invalide');
        }

        $request = &$_SESSION['register_requests'][$email];

        if ($request['retry'] >= self::MAX_RETRY) {
            unset($_SESSION['register_requests'][$email]);
